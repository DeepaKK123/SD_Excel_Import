SUBROUTINE SR.SUNBIZ.FIC.DICT.BUILD(DATA.FILE, NEXT.ATTR, ERR.MSG)
*=============================================================================
* SR.SUNBIZ.FIC.DICT.BUILD
*
* Purpose:
*   Same job as the generic SR.DICT.BUILD, but sized to the actual SunBiz
*   Fictitious Name Data File layout: each DICT item's display FORMAT
*   matches that field's real fixed-width length (SR.SUNBIZ.FIC.LENGTHS)
*   instead of a generic guess. Column names come from
*   SR.SUNBIZ.FIC.HEADER; column 1 is the Record ID (@ID), columns 2+
...
*=============================================================================
   ...
   CALL SR.SUNBIZ.FIC.HEADER(HEADER.ROW)
   CALL SR.SUNBIZ.FIC.LENGTHS(LENGTHS.ROW)
